...
// Icons
import { IconGithub, IconOpenProject } from "@components/Icons.jsx";
...
function CardCreations({
  ...
  return (
    <div
      id="hisy"
      className={`card card-side bg-base-100 shadow-sm max-sm:flex-col-reverse ${imgReverse} mb-8 `}
    >
      <div className="card-body">
        ...
        <div>
          {!!others?.length &&
            others.map((other, i) => (
              <div key={i} className="badge badge-neutral mr-1">
                {other}
              </div>
            ))}
        </div>
        <div className="card-actions justify-end">
          {github && (
            <Link target="_blank" to={github} className="btn btn-ghost">
              <IconGithub />
            </Link>
          )}
          {site && (
            <Link target="_blank" to={site} className="btn btn-ghost">
              <IconOpenProject />
            </Link>
          )}
